...
class WorkPermission(ModelBase):
    ...
    date_from = models.DateTimeField(verbose_name='Fecha desde', default=datetime.date.today())
    date_to = models.DateTimeField(verbose_name='Fecha hasta')
    time_earring = models.IntegerField(verbose_name="Tiempo Pendiente", default = 0 )
    status = models.BooleanField(verbose_name="Procesado",default=False)
    active = models.BooleanField(verbose_name='Activo', default=True)
    
    def save(self, *args, **kwargs):
        from datetime import datetime
        self.date_to = self.date_from + timedelta(days=self.time_permission) if self.date_from and self.time_descuent else None
