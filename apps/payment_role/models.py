...
class Item(ModelBase):
    ...
    sucursal = models.ForeignKey(
        Organization, on_delete=models.PROTECT, verbose_name='sucursal')
    type_item = models.IntegerField(choices=TYPE_ITEM, default=TYPE_ITEM[0][0])
    collective = models.BooleanField(default=False)
    role_frequency = models.ManyToManyField(
        RoleFrequency, verbose_name='Frecuencia Rol')
    order = models.IntegerField(verbose_name="Orden")
    discount = models.IntegerField(
        choices=TYPE_DISCOUNT, default=TYPE_ITEM[0][0])
    value = models.DecimalField(
        verbose_name="Valor", decimal_places=2, max_digits=18)
    role_month = models.BooleanField(verbose_name="Rol mensual", default=False)
    thirteenth = models.BooleanField(
        verbose_name="Decimo Tercero", default=False)
    fourteenth = models.BooleanField(
        verbose_name="Decimo Cuarto", default=False)
    vacation = models.BooleanField(verbose_name="Vacacion", default=False)
    reserve_fund = models.BooleanField(
        verbose_name="Fondo Reserva", default=False)
    ...
    employer_retirement = models.BooleanField(
        verbose_name="Liquidacion Patronal", default=False)
    Hours_extras = models.BooleanField(
        verbose_name="Horas Extras", default=False)
...
class Calendar(ModelBase):
    year = models.IntegerField(
        verbose_name='Año', default=datetime.date.today().year)
    month = models.IntegerField(
        verbose_name='Mes', default=datetime.date.today().month)
    role_frequency = models.ForeignKey(
        RoleFrequency, on_delete=models.PROTECT, verbose_name='Frecuencia Rol')
...
class Overtime(ModelBase):
    calendar = models.ForeignKey(
        Calendar, on_delete=models.PROTECT, verbose_name='Calendario Rol')
    employee = models.ForeignKey(
        Employee, on_delete=models.PROTECT, verbose_name='Empleado')
    sucursal = models.ForeignKey(
        Organization, on_delete=models.PROTECT, verbose_name='Sucursal')
    value_hour = models.DecimalField(
        verbose_name="Valor hora", decimal_places=2, max_digits=10)
    total = models.DecimalField(
        verbose_name="Total Sobretiempo", decimal_places=2, max_digits=10)
    processed = models.BooleanField(default=False)
    calendar_process = models.IntegerField(
...
class OvertimeDetail(ModelBase):
    overtime = models.ForeignKey(
        Overtime, on_delete=models.CASCADE, verbose_name='Sobretiempo')
    item = models.ForeignKey(Item, on_delete=models.PROTECT,
                             verbose_name='Tipo Horas', blank=True, null=True)
    number_hours = models.DecimalField(
        verbose_name="Numero horsa", decimal_places=2, max_digits=10)
    value = models.DecimalField(
        verbose_name="Valor", decimal_places=2, max_digits=10)
...
class PayRoll(ModelBase):
    ...
    income = models.DecimalField(
        verbose_name="Total Ingresos", decimal_places=2, max_digits=18)
    discounts = models.DecimalField(
        verbose_name="Total Descuentos", decimal_places=2, max_digits=18)
    net = models.DecimalField(
        verbose_name="Neto Recibir", decimal_places=2, max_digits=18)
    processed = models.BooleanField(default=False)
...
class PayRollDetail(ModelBase):
    payRoll = models.ForeignKey(
        PayRoll, on_delete=models.PROTECT, verbose_name='Nomina')
    employee = models.ForeignKey(
        Employee, on_delete=models.PROTECT, verbose_name='Empleado')
    income = models.DecimalField(
        verbose_name="Total Ingresos", decimal_places=2, max_digits=10)
    discounts = models.DecimalField(
        verbose_name="Total Descuentos", decimal_places=2, max_digits=10)
    net = models.DecimalField(
        verbose_name="Neto Recibir", decimal_places=2, max_digits=10)
...
class PayRollDetailItem(ModelBase):
    payRollDetail = models.ForeignKey(
        PayRollDetail, on_delete=models.PROTECT, verbose_name='Detalle Rubros')
    item = models.ForeignKey(
        Item, on_delete=models.PROTECT, verbose_name='Rubro')
    value = models.DecimalField(
        verbose_name="Valor", decimal_places=2, max_digits=10)
...
class EmployeeItems(ModelBase):
    employee = models.ForeignKey(
        Employee, on_delete=models.PROTECT, verbose_name='Empleado')
    item = models.ForeignKey(Item, verbose_name='Rubro',
...
class TypePermission(ModelBase):
    ...
    time_limit = models.DecimalField(
        verbose_name="Tiempo Permitido(dias/horas)", decimal_places=2, max_digits=10)
    days25 = models.IntegerField(verbose_name='Dias 25%', default=0)
    classPermission = models.IntegerField(
        verbose_name="Clase Permiso", choices=CLASS_PERMISSION, default=CLASS_PERMISSION[0][0])
    remuneration = models.BooleanField(
        verbose_name='Remunerado', default=False)
...
class WorkPermission(ModelBase):
    employee = models.ForeignKey(
        Employee, on_delete=models.PROTECT, verbose_name='Empleado')
    type_permission = models.ForeignKey(
        TypePermission, on_delete=models.PROTECT, verbose_name='Tipo Permiso')
    time_descuent = models.IntegerField(
        verbose_name="Permiso en", choices=TIME_DESCUENT, default=[0][0])
    time_permission = models.IntegerField(verbose_name="Tiempo Permiso")
    date_from = models.DateTimeField(
        verbose_name='Fecha desde', default=timezone.now)
    date_to = models.DateTimeField(verbose_name='Fecha hasta')
    time_earring = models.IntegerField(
        verbose_name="Tiempo Pendiente", default=0)
    ...
    def save(self, *args, **kwargs):
        self.date_to = self.date_from + \
            timedelta(
                days=self.time_permission) if self.date_from and self.time_descuent else None
        self.time_earring = self.time_permission if self.time_permission else None
