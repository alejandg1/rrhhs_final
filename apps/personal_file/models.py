...
class KinshipType(ModelBase):
    kinship = models.CharField(
        verbose_name='Parentesco', max_length=100, unique=True)
...
class MaritalStatus(ModelBase):
    estado_civil = models.CharField(
        verbose_name='Estado civil', max_length=50, unique=True)
...
class Category(ModelBase):
    ...
    salary = models.DecimalField(
        verbose_name="Sueldo", decimal_places=2, max_digits=18)
    active = models.BooleanField(verbose_name='Activo', default=True)
...
class TypeEmployee(ModelBase):
    name = models.CharField(verbose_name='Descripcion',
...
class Area(ModelBase):
    name = models.CharField(verbose_name='Nombre', max_length=50, unique=True)
    type_area = models.ForeignKey(
        TypeArea, on_delete=models.PROTECT, verbose_name='type_area')
    sucursal = models.ForeignKey(
        Organization, on_delete=models.PROTECT, verbose_name='Sucursal')
    predecessor = models.ForeignKey(
        "self", on_delete=models.PROTECT, verbose_name='Predecesor', blank=True, null=True)
    active = models.BooleanField(verbose_name='Activo', default=True)
...
class Employee(ModelBase):
    ...
    image = models.ImageField(
        verbose_name='Foto', upload_to='empleado', max_length=500, null=True, blank=True)
    ...
    city = models.ForeignKey(
        City, on_delete=models.PROTECT, verbose_name='Ciudad', blank=True, null=True)
    direction = models.CharField(verbose_name="Dirección", max_length=100)
    latitude = models.CharField(
        verbose_name="Latitud", max_length=100, blank=True, null=True)
    longitude = models.CharField(
        verbose_name="Longitud", max_length=100, blank=True, null=True)
    sucursal = models.ForeignKey(
        Organization, on_delete=models.PROTECT, verbose_name='Sucursal', blank=True, null=True)
    marital_status = models.ForeignKey(
        MaritalStatus, on_delete=models.PROTECT, verbose_name='Estado Civil', blank=True, null=True)
    date_admission = models.DateTimeField(
        verbose_name="Fecha Ingreso", default=now)
    post = models.ForeignKey(
        Post, on_delete=models.PROTECT, verbose_name='Cargo', blank=True, null=True)
    category = models.ForeignKey(
        Category, on_delete=models.PROTECT, verbose_name='Categoria', blank=True, null=True)
    area = models.ForeignKey(
        Area, on_delete=models.PROTECT, verbose_name='Area', blank=True, null=True)
    employee_class = models.IntegerField(
        verbose_name="Clase Empleado", choices=EMPLOYEE_CLASS, default=EMPLOYEE_CLASS[1][0])
    type_regime = models.ForeignKey(
        TypeRegime, on_delete=models.PROTECT, verbose_name='Tipo Regimen', blank=True, null=True)
    type_employee = models.ForeignKey(
        TypeEmployee, on_delete=models.PROTECT, verbose_name='Tipo Empleado', blank=True, null=True)
    type_contract = models.ForeignKey(
        TypeContract, on_delete=models.PROTECT, verbose_name='Tipo Contrato', blank=True, null=True)
    number_iess = models.CharField(
        verbose_name="Numero Iess", max_length=50, blank=True, null=True)
    ...
    frequency_rol = models.IntegerField(
        verbose_name="Frecuencia Rol", choices=FREQUENCY_ROL, default=FREQUENCY_ROL[1][0], blank=True, null=True)
    ...
    def get_model_dict(self):
        item = model_to_dict(self, exclude=['QR'])
    ...
    def generate_qr(self):
        ...
        img_path = os.path.join(media_path, f'qrcode_{self.dni}.png')
        img.save(img_path)
        return f'qrcodes/qrcode_{self.dni}.png'

    def get_qr_image_url(self):
        return utils.get_image(self.QR)
...
class Dependent(ModelBase):
    ...
    employee = models.ForeignKey(
        Employee, on_delete=models.CASCADE, verbose_name='Empleado Responsable')
    kinship = models.ForeignKey(
        KinshipType, on_delete=models.PROTECT, verbose_name='Parentesco')
    marital_status = models.ForeignKey(
        MaritalStatus, on_delete=models.PROTECT, verbose_name='Estado Civil')
    ...
    fecha_nacimiento = models.DateTimeField(
        verbose_name='Fecha de nacimiento', default=now)
    country = models.ForeignKey(
        Country, on_delete=models.PROTECT, verbose_name='Pais')
    city = models.ForeignKey(
...
class Medicaldata(ModelBase):
    empleado = models.ForeignKey(
        Employee, verbose_name='Empleado', on_delete=models.CASCADE)
    tipo_sangre = models.CharField(
        verbose_name='Tipo de sangre', max_length=10)
    ...
    Ultima_revision = models.DateField(
        verbose_name='Ultima revision', default=now)
    discapacidad = models.BooleanField(
        verbose_name="Discapacidad", default=False)
    des_discapacidad = models.TextField(
        verbose_name='descripcion de Discapacidad', blank=True)
